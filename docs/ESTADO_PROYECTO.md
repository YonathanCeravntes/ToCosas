...
**FIN-030 — Umbrella SO Financiero (Deudas por tipo + movimientos inteligentes) — DECIDIDO.** ✅ ARQ-0030 (`2a18a76`) · ✅ AUD-0030 · ✅ **DEC-0030 (umbrella)**. Aprobada la espina (Alt A + capa de consecuencias por listeners con causalidad sobre outbox FIN-002 → §42 por construcción). Sin IMP propio. → derivó **FIN-031 (Fase 1)**, ahora la activa: espina + compra-con-tarjeta e2e, `ARQ-0031` en curso. Condición dura de FIN-031: política de reversión con dependientes (borde §42). `FIN-032` (resto de tipos) y `FIN-033` (confirmación mensual) en roadmap. `docs/oficial/DEC-0030-SO-Financiero-Deudas-Movimientos.md`.
