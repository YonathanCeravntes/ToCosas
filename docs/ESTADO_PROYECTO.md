...
- **FIN-029 — Telegram / Motor Conversacional único.** ✅ ARQ-0029 · ✅ AUD-0029 · ✅ **DEC-0029 emitida — `IMP-0029` habilitado (último de la tanda).** CPSAO dio visto con 4 condiciones (acuse explícito; honestidad al no entender; barrera de Independencia para tools de consejo; paywall honesto) + test PII por tool + gate DPA+PIA intacto. Motor único y dedupe `update_id` ya existen (verificados) → probar, no duplicar. **En espera del `IMP-0029` del Arquitecto.** `docs/oficial/DEC-0029-Integracion-Telegram.md`.
