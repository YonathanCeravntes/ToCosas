...
**FIN-030 — Umbrella SO Financiero (Deudas por tipo + movimientos inteligentes) — DECIDIDO.** ✅ ARQ-0030 (`2a18a76`) · ✅ AUD-0030 · ✅ **DEC-0030 (umbrella)**. Aprobada la espina (Alt A + capa de consecuencias por listeners con causalidad sobre outbox FIN-002 → §42 por construcción). Sin IMP propio. → derivó **FIN-031 (Fase 1)**, ahora la activa: espina + compra-con-tarjeta e2e. ✅ ARQ-0031 · ✅ AUD-0031 (GAP §32) · ✅ **DEC-0031 — `IMP-0031` habilitado.** Cambio obligatorio principal: extender `DebtOutlayService` (autoridad única de "lo comprometido") para incluir `CardInstallment` en UNA ruta. Política de reversión §4.5 (Alt A) aprobada; grep §32 + test de reversibilidad como cierre. En espera del `IMP-0031` del Arquitecto. `FIN-032` (resto de tipos) y `FIN-033` (confirmación mensual) en roadmap. `docs/oficial/DEC-0030-SO-Financiero-Deudas-Movimientos.md`.
