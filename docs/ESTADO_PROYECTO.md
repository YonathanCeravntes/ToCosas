...
## Frentes en diseño (ARQ en paralelo — excepción documentada a "un FIN a la vez")
Autorizados por el Fundador (memo 2026-07-13) durante la espera del build OTA. Solo la **fase de diseño (ARQ)** corre en paralelo; el IMP sigue secuencial y no adelanta el cierre de FIN-026.
- **FIN-027 — Modelo de ingresos personales.** ARQ en curso, directiva del CTO emitida (`docs/correspondencia/FIN-027-Modelo-de-Ingresos.md`). Requisito duro: base de deducción configurable (total/parcial) + salario fijo y variable coexistentes; ingreso neto disponible como única definición §32.
- **FIN-028 — Gestión de movimientos (editar/anular).** ARQ en curso — **retención levantada**: el Fundador entregó sus 10 observaciones sobre el módulo Registrar (memo 2026-07-13), directiva `ARQ-0028` emitida. Requisitos duros: anulación lógica (nunca borrado físico), sin lógica financiera en la mutación (recálculo por el Motor vía bus de eventos §32/FIN-002), servicio central único de movimientos reutilizable por Telegram/WhatsApp (liga FIN-029), modelo preparado para auditoría/IA futura. `docs/correspondencia/FIN-028-Gestion-de-Movimientos.md`.
- **FIN-029 — Telegram / Motor Conversacional único.** ARQ en curso, directiva emitida (`docs/correspondencia/FIN-029-Integracion-Telegram.md`). Un solo motor conversacional reutilizable Telegram/WhatsApp; desacople Motor Financiero↔Conversacional; datos reales de IA bloqueados por gate DPA+PIA.
