...
- **FIN-028 — Gestión de movimientos (editar/anular).** ✅ ARQ-0028 (`7cdbdf0`) · ✅ AUD-0028 · ✅ **DEC-0028 emitida — `IMP-0028` habilitado (1º de la secuencia).** Corrección de diseño del CTO: la anulación **es** `deletedAt` (ya existe), NO se añade `anulada`/`voidedAt` (§32). Bug confirmado: `update`/`remove` sin outbox → se envuelven en el patrón outbox (Motor recalcula por el listener ya existente). `ACTIVE_TX_FILTER` solo sobre `Transaction`, verificado por diff. `docs/oficial/DEC-0028-Gestion-de-Movimientos.md`. **En espera del `IMP-0028` del Arquitecto.**
- **FIN-027 — Modelo de ingresos personales.** ✅ ARQ-0027 (`0b1b044`) · ✅ AUD-0027 · ⏳ **DEC retenida (turno 2).** Decisión de producto pendiente: DTI/Score sobre neto o bruto (posible consulta al CPSAO) + migración FixedItem→fuentes. `NetIncomeService` hoja §32 + `withheldAtSource`. Se decide tras cerrar `IMP-0028`. `docs/correspondencia/FIN-027-Modelo-de-Ingresos.md`.
- **FIN-029 — Telegram / Motor Conversacional único.** ✅ ARQ-0029 (`c710e2d`) · ✅ AUD-0029 · ⏳ **DEC retenida (turno 3, depende del servicio central de FIN-028).** Hallazgo: el motor agnóstico y el dedupe por `update_id` ya existen — verificar/formalizar, no reconstruir. Gate DPA+PIA intacto. `docs/correspondencia/FIN-029-Integracion-Telegram.md`.
