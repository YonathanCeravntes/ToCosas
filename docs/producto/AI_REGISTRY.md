...
|----|--------|----------|--------|---------------------|------------------------------|--------------------------|------------------------|----------------|---------------------------|---------------------------|
| AI-0001 | CPSAO (Chief Product, Strategy & AI Officer) | Estrategia de producto, ideación, roadmap 1/3/5 años | `oficial` (regularizado) | 2026-07-06 | Fundador | `docs/producto/*`, `docs/roadmap/BACKLOG.md` (solo lectura) | `IDEA-XXXX.md`, propuestas estratégicas | No diseña arquitectura, no programa, no modifica el Backlog, no implementa, no tiene autoridad jerárquica sobre el CTO | No aplica — no es tool de LLM, no accede a datos de usuario | `ESTADO_PROYECTO.md` · `PRODUCT_VISION.md` · `BACKLOG.md` (índice) |
...
- 2026-07-12 — CTO: corrección — `docs/correspondencia/*` (mensajería entre roles,
  propuesta del CPSAO) **no es** un documento de gobernanza ni afecta el alcance
  registrado en esta tabla. Aclaración del Fundador: es un canal de comunicación
  entre las IA del proyecto, no un permiso de escritura sobre artefactos oficiales
  ni sobre el producto. Se retira de esta tabla; el mecanismo se documenta solo en
  `docs/correspondencia/` mismo.
