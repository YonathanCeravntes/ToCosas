/**
 * FIN-026: capturas full-scroll del Simulador.
 *  - antes:   Deudas → "Verlo en el simulador" → pantalla vieja.
 *  - despues: mismo camino (precarga extra=0 + piso/techo) → Simular →
 *             resultado narrado + puente; chip de abono (selector con la deuda
 *             recomendada); chip de venta con demo.mora (estado vacío).
 * Uso: APP_URL=http://localhost:<puerto> node capture-fin026.js <carpeta> <prefijo>
 */
...
(async () => {
  const laura = await session('demo.laura@millo.app', 'Demo2026!millo');

  const PROFILE = process.env.TEMP + '/millo-fin026-' + Date.now();
  ...
  const seed = async (s) => {
    await evalJs(`
      localStorage.clear();
      localStorage.setItem('tocosas.tokens', ${JSON.stringify(JSON.stringify(s.tokens))});
      localStorage.setItem('tocosas.user', ${JSON.stringify(JSON.stringify(s.user))});
      'seeded'`);
  ...
  // Camino real: Deudas → "Verlo en el simulador →".
  console.log('tab deudas:', await clickText('Deudas'));
  await wait(4000);
  console.log('a simulador:', await clickText('Verlo en el simulador'));
  await wait(5000);
  await fullShot(`fin026-${PREFIX}-estrategia-llegada.png`);

  if (PREFIX === 'despues') {
    // Simular con el 0 precargado → resultado narrado + puente.
    console.log('simular:', await clickText('Simular'));
    await wait(6000);
    await fullShot('fin026-despues-estrategia-resultado.png');

    // Chip de abono: selector con la deuda recomendada preseleccionada.
    console.log('chip abono:', await clickText('abono extra'));
    await wait(2500);
    await fullShot('fin026-despues-abono-selector.png');

    // Estado vacío (venta) con la usuaria sin activos.
    const mora = await session('demo.mora@millo.test', 'Demo2026!mora');
    await setViewport(844);
    await seed(mora);
    console.log('tab deudas:', await clickText('Deudas'));
    await wait(4000);
    console.log('a simulador:', await clickText('Verlo en el simulador'));
    await wait(5000);
    console.log('chip venta:', await clickText('vendo un activo'));
    await wait(2500);
    await fullShot('fin026-despues-venta-vacio.png');
  }

  ws.close();
  edge.kill();
  console.log('FIN-026 CAPTURADO (' + PREFIX + ')');
  process.exit(0);
