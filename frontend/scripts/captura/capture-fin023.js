/**
 * FIN-023: capturas full-scroll — Deudas (hero con desembolso real), detalle de
 * la Tarjeta (sección "Seguros y cargos" con cuota de manejo) y Presupuesto
 * (cuotas comprometidas con la nota condicional).
 * Uso: APP_URL=http://localhost:<puerto> node capture-fin023.js <carpeta>
 */
...
(async () => {
  ...
  await wait(4000);
  await fullShot('fin023-01-deudas-lista.png');

  console.log('detalle tarjeta:', await clickText('Tarjeta de crédito'));
  await wait(5000);
  await fullShot('fin023-02-detalle-tarjeta.png');

  console.log('tab presupuesto:', await clickText('Presupu'));
  await wait(4000);
  await fullShot('fin023-03-presupuesto.png');

  ws.close();
  edge.kill();
  console.log('FIN-023 CAPTURADO');
  process.exit(0);
